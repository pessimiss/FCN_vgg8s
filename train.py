...
# 对第一2X上采样后的网络进行再次2x上采样
# output_shape为pool3之后的网络施加1x1卷积的输出aux_logits_8s
upsample_filter_np_x2_2 = bilinear_upsample_weights(2,  
                                                  number_of_classes)

upsample_filter_tensor_x2_2 = tf.Variable(upsample_filter_np_x2, name='vgg_16/fc8/t_conv_x2')
upsampled_logits = tf.nn.conv2d_transpose(upsampled_logits, upsample_filter_tensor_x2_2,
                                          output_shape=tf.shape(aux_logits_8s),
                                          strides=[1, 2, 2, 1],
                                          padding='SAME')

# 经过2次2X上采样的网络 + pool3后网络之卷积网络
